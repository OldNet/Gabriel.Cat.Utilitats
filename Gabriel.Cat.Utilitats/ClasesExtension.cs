...
namespace Gabriel.Cat.Extension
{
	...
	public static class Extension
	{
		...
		public static Vector GetVector<T>(this T[,] matriz, int angulo = 45, int xInicio = 0, int yInicio = 0, int longitud = -1)
		{
			...
			if (xInicio == 0 && yInicio == 0 || xInicio == width && yInicio == 0 || yInicio == height && xInicio == width || yInicio == height && xInicio == 0) {//si esta en una esquina :D
				alfa = angulo % 90;
				if (alfa > 45) {
					ladoContiguo = width;
				} else {
			...
			} else if (xInicio == 0 || yInicio == 0 || xInicio == width || yInicio == height) {//si  esta en un borde :D
				alfa = angulo % 180;
				if (alfa > 90) {
