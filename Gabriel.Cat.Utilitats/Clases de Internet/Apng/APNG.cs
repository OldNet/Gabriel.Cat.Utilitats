...
namespace Gabriel.Cat
{
	...
	public class APNG:IEnumerable<PNG>
	{
		...
		IHDRChunk ihdrChunk;
		bool saltarPrimerFotograma = false;
		IENDChunk iendChunk;
		uint numeroDeRepeticiones;
		public APNG()
		{
			pngs = new List<PNG>();
		...
		public uint NumeroDeRepeticiones {
			get{ return numeroDeRepeticiones; }
			set{ numeroDeRepeticiones = value; }
		}
		public bool SaltarPrimerFotograma {
			get {
				return saltarPrimerFotograma;
			}
			set {
				saltarPrimerFotograma = value;
			}
		}
		...
		public Bitmap ToBitmap(int index)
		{
			...
			if (index > Count)
				throw new ArgumentException(String.Format(
					"Embedded PNG index must be between 0 and {0}", Count - 1));
			// Create the bitmap
			return pngs[index].ToBitmap();
		}
		public Bitmap[] ToBitmapArray()
		{
			Llista<Bitmap> bmps = new Llista<Bitmap>();
			for (int i = 0; i < Count; i++)
				bmps.Afegir(ToBitmap(i));
			return bmps.ToTaula();
		}

		#if (DEBUG)
		/// <summary>
		/// Save a Fragment in png format
		/// </summary>
			...
			//	pngs.Ordena();
				...
				for (int i = 1; i < pngs.Count; i++) {
					...
					for (int j = 0; j < pngs[i].IDATS.Count; j++) {
						...
						chunkIDAT.ChunkType = fdATChunk.NAME;// los demas fdAT
		...
		public void Load(string filename)
		{
			...
			try {
				...
				if (chunk.ChunkType != IHDRChunk.NAME)
					throw new Exception("Error de formato se encuentra " + chunk.ChunkType + " y se esperaba " + IHDRChunk.NAME);
				AñadirChunk(chunk);
				//leo el acTL
				chunk = Chunk.ReadChunk(stream);

				if (chunk.ChunkType != acTLChunk.NAME)
					throw new Exception("Error de formato se encuentra " + chunk.ChunkType + " y se esperaba " + acTLChunk.NAME);
				...
				//	png.FCTL.SequenceNumber=0;
					...
					throw new Exception("Error de formato se encuentra " + chunk.ChunkType + " y se esperaba " + fcTLChunk.NAME);
				else {
